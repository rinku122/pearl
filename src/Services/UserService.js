...
const saveStepFirst = (data) => {
  return fetch('post', `${API_HOST}paiid/saveStep1Form`, data, {
    userLang: userOverrideLang,
  });
};
const validateToken = (data) => {
  return fetch('post', `${API_HOST}paiid/validateToken`, data, {
    userLang: userOverrideLang,
  });
};
const resendEmailVerification = (data) => {
  return fetch('post', `${API_HOST}paiid/resendEmail`, data, {
    userLang: userOverrideLang,
  });
};
...
const getMerchantDetails = (name) => {
  return fetch(
    'get',
    `${API_HOST}paiid/merchantDetails/${name}`,
    {},
    { userLang: userOverrideLang }
  );
};
...
const userDetails = (data) => {
  return fetch('post', `${API_HOST}Transaction/registration`, data);
};
